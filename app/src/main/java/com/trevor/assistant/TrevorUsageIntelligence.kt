...
object TrevorUsageIntelligence {
    fun hasAccess(context: Context): Boolean {
        ...
        return manager.queryUsageStats(UsageStatsManager.INTERVAL_DAILY, now - 86400000L, now)
            .any { it.totalTimeInForeground > 0L }
    }
    ...
    fun analyze(context: Context, days: Int = 7): TrevorUsageAnalysis {
        ...
        val manager = context.getSystemService(UsageStatsManager::class.java)
            ?: return TrevorUsageAnalysis(false, "Usage statistics are unavailable.", emptyList())
        ...
        val summary = buildString {
            ...
            if (top.isNotEmpty()) append(" Most-used app: ").append(top.first().packageName)
                .append(" (~").append(top.first().minutes).append(" min).")
            if (peak != null) append(" Peak recorded hour: ")
                .append(String.format(Locale.ROOT, "%02d:00", peak)).append(".")
        }
...
") {
    ...
    fun compactContext(context: Context): String {
        val a = analyze(context)
        if (!a.available) return "Usage intelligence unavailable until Usage Access is enabled."
        return a.summary + " Top apps: " + a.topApps.take(5).joinToString(", ") {
            it.packageName + " " + it.minutes + "m"
        }
    }
}
