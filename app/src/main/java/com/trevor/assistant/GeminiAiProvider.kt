            ...
            val schema = JSONObject()
                ...
                .put("properties", JSONObject()
                    .put("kind", JSONObject().put("type", "string").put("enum", JSONArray()
                        ...
                        .put("AUTOMATION").put("TERMINAL").put("MEMORY").put("REMINDER").put("ROUTINE").put("USAGE").put("GENERAL")))
                    .put("mode", JSONObject().put("type", "string").put("enum", JSONArray().put("NORMAL").put("ANALYSE").put("RESEARCH").put("PROJECT").put("RATIO_SHIFTER").put("TERMINAL")))
                    ...
                    .put("actions", JSONObject().put("type", "array").put("maxItems", 20).put("items", JSONObject()
                        .put("type", "object")
                        .put("properties", JSONObject()
                            .put("action", JSONObject().put("type", "string"))
                            .put("argument", JSONObject().put("type", "string"))
                            .put("requiresForeground", JSONObject().put("type", "boolean"))
                            .put("verify", JSONObject().put("type", "string")))
                        .put("required", JSONArray().put("action").put("argument").put("requiresForeground").put("verify"))
                        .put("additionalProperties", false)))
                .put("required", JSONArray().put("kind").put("mode").put("confidence").put("needsFreshInformation").put("needsAdvancedModel").put("actions"))
                .put("additionalProperties", false)
            val system = "Classify the user's meaning, not keywords. Understand natural phrasing, slang, implied intent, misspellings and conversational requests. Choose exactly one intent and one execution mode. Intents: QUESTION, EXPLANATION, RESEARCH, ANALYSIS, PROJECT, AUTOMATION, TERMINAL, MEMORY, REMINDER, ROUTINE, USAGE, GENERAL. Modes: NORMAL, ANALYSE, RESEARCH, PROJECT, RATIO_SHIFTER, TERMINAL. RESEARCH requires current or externally verified information. AUTOMATION means the user wants a supported device action. TERMINAL means shell/terminal work. MEMORY saves or recalls information. REMINDER schedules something. ROUTINE reports learned recurring behaviour. USAGE reports local app-usage patterns. ANALYSIS inspects, debugs, compares or diagnoses. PROJECT is software/project work. EXPLANATION teaches a concept. QUESTION is ordinary information seeking. GENERAL is conversation or anything else. Set the mode to the user's actual goal even when they do not name a mode. Use RATIO_SHIFTER only for UI, layout, or responsive-ratio requests. Set needsFreshInformation when current/external verification is needed. For AUTOMATION, populate actions with only supported TREVOR actions and their arguments. Never invent unsupported actions. Use an empty actions array when no safe supported action can be represented. Set needsAdvancedModel for research, analysis, project, terminal, complex automation, attachments or unusually complex requests. Return JSON only; never output reasoning or hidden thoughts."
            val body = JSONObject()
