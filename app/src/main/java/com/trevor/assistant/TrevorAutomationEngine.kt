...
data class TrevorAutomationStep(val action: String, val argument: String = "", val requiresForeground: Boolean = false)
...
object TrevorAutomationEngine {
    ...
    private fun parseStep(text: String): TrevorAutomationStep? {
        ...
        return when {
            lower.contains("open wi-fi") || lower.contains("open wifi") -> TrevorAutomationStep("OPEN_WIFI", requiresForeground = true)
            lower.contains("open bluetooth") -> TrevorAutomationStep("OPEN_BLUETOOTH", requiresForeground = true)
            lower.contains("open display settings") -> TrevorAutomationStep("OPEN_DISPLAY", requiresForeground = true)
            lower.contains("open sound settings") -> TrevorAutomationStep("OPEN_SOUND", requiresForeground = true)
            lower.contains("open battery") || lower.contains("battery saver") -> TrevorAutomationStep("OPEN_BATTERY", requiresForeground = true)
            lower.contains("open notification settings") -> TrevorAutomationStep("OPEN_NOTIFICATIONS", requiresForeground = true)
            lower == "open settings" || lower.endsWith(" open settings") -> TrevorAutomationStep("OPEN_SETTINGS", requiresForeground = true)
            lower.startsWith("analyse my usage") || lower.startsWith("analyze my usage") -> TrevorAutomationStep("USAGE_SUMMARY")
            lower.startsWith("share ") -> TrevorAutomationStep("SHARE_TEXT", text.substringAfter(" ").trim(), true)
            else -> null
        }
    }
    fun execute(context: Context, plan: TrevorAutomationPlan): String {
        ...
        for ((index, step) in plan.steps.withIndex()) {
            ...
            if (!result.startsWith("OK")) return "TREVOR task stopped after step " + (index + 1) + ".\n" + results.joinToString("\n")
        }
